...
// 		$("#apply_to_date").attr("readonly", true);
	...
	function apply(type){	//신청 팝업
		...
		$.ajax({
			...
			data : {emp_seq : '${userInfo.empSeq}', apply_from_date : '${year}'+'0101', apply_to_date : '${year}'+'1231', skip:0, pageSize:100},
			type : "POST",
			async : false,
			success : function(result){
				familyData = result.list.filter(function(e){return e.status == 1});
			}
		});
...
// 			$("#gridRowAdd").show();
			...
			$.each(familyData, function(i,v){
				...
				oneRow += '<input type="hidden" id="user_welfare_id'+$addIndex+'" name="user_welfare_id" value="'+v.user_welfare_id+ '">';
				...
				$("#family_relationship"+$addIndex).kendoDropDownList({
					...
					enable : false
				}).data("kendoDropDownList").text(v.family_relationship);
...
// 							if(dataItem.welfare_apply_id != undefined || dataItem.welfare_apply_id != null){
// 								if(dataItem.status_name == '신청'){
// 									return "<span class = 'welfare_apply_id' ondblclick = 'dbclickToApply()'>"+dataItem.welfare_apply_id+"</span>"
// 								}else{
// 									return "<span class = 'welfare_apply_id'>"+dataItem.welfare_apply_id+"</span>"
// 								}
// 							}else{
// 								return "<span class = 'welfare_apply_id'></span>"
// 							}
// 						},
// 					title : "신청번호"
// 				},
...
// 						}
	...
	function save(sep, status){
		...
		if($insertChk){		//status : insert
			...
			param.status = "update";
			param.welfare_apply_id = $welfare_apply_id;
				var data = [];
				var apply_family_name = '';
				$(".personDetail").each(function(i,v){
					var list = {};
			...
			success : function(result){
				...
				if(sep == 'click'){
					if(result.status == "insert"){
						...
						formData.append('target_table_name','welfare_file_attach');
...
<div class="iframe_wrap">
	<div class="sub_title_wrap">
		<div class="title_div">
			<h4>복지포인트신청(신규, 자격상실)</h4>
		</div>
	</div>
	<div class="sub_contents_wrap">
		<div class="btn_div mt10 cl">
			<div class="left_div">
				<p class="tit_p mt5 mb0">복지포인트신청(신규, 자격상실)</p>
			</div>
			<div class="right_div">
...
<!-- 				</dd> -->
		...
		<div class="btn_div mt10 cl">
		<div class="left_div"><span class="text_red pl20">자녀의 경우 20세미만만 신청가능합니다.</div>
			<div class="right_div">
				<div class="controll_btn p0">
		...
		<div class = "top_box mt5">
			<dl class="bg_skyblue2" style="background-color: #CEF6F5">
				<dt class="ml5" style="width: 170px">
					<img src="<c:url value='/Images/ico/ico_alert.png'/>" alt="alertIcon" />&nbsp;&nbsp;복지포인트신청 안내
				</dt>
			</dl>
