...
function fnBiddingInit(){
	...
	$.ajax({
		...
		success : function(data) {
			...
			$('#bidding_rebidding_title').val(data.biddingReferInfo.title);
			$('#bidding_result').kendoDropDownList();
	...
	if(approKey){
		...
		$.ajax({
			...
			dataType : 'json',
			success : function(data) {
				$('#bidding_rebidding_title').val(data.biddingInfo.title);
...
function fnApproval(){
	...
	data.title = $('#bidding_rebidding_title').val();
	...
	$.ajax({
		...
		success : function(data) {
		    ...
		    params.approKey = 'PURCBID3_' + purcReqId + '_' + $('#bidding_type').val() + '_' +  + data.purc_req_bidding_id;
		    params.outProcessCode = 'PURCBID3';
		    params.empSeq = $('#empSeq').val();
		    params.mod = 'W';
		    params.fileKey = makeDjFileKey();
		    onnaraFileToTemp(params.fileKey);
		    params.contentsStr = makeContentsStr();
		    params.title = $('#bidding_title').val() + ' 입찰 결과와 재공고';
...
function makeContentsStr(){
	...
	html += '.';
	html += '<br/>';
	html += '2. ' + $('#bidding_title').val() + '이 유찰됨에 따라 재공고를 실시하고자 합니다.';
	html += '<br/>';
	html += '<br/>';
	html += '&nbsp;&nbsp;가. 입찰 내용';
	html += '<br/>';
	...
	html += '<br/>';
	html += '&nbsp;&nbsp;&nbsp;&nbsp;2) 사업예산 : 금' + $('#bidding_budget_am').val() + '원(금 ' + numToKOR($('#bidding_budget_am').val().toMoney2()) + '원) 부가세 포함';
	html += '<br/>';
	html += '&nbsp;&nbsp;&nbsp;&nbsp;3) 수행부서 : ' + $('#bidding_dept').val();
	html += '<br/>';
	html += '&nbsp;&nbsp;&nbsp;&nbsp;4) 공고방법 : ' + $('#bidding_way').val();
	html += '<br/>';
	html += '&nbsp;&nbsp;&nbsp;&nbsp;5) 입찰기간 : ' + moment($('#bidding_fr_dt').val()).format('YYYY.MM.DD') + ' ~ ' + moment($('#bidding_to_dt').val()).format('YYYY.MM.DD');
	html += '<br/>';
	html += '&nbsp;&nbsp;&nbsp;&nbsp;6) 입찰결과 : ' + $('#bidding_result').data('kendoDropDownList').text();
	html += '<br/>';
	html += '&nbsp;&nbsp;&nbsp;&nbsp;7) 유찰사유 : ' + $('#bidding_reason').val();
	html += '<br/>';
	html += '&nbsp;&nbsp;나. 유찰에 따른 재공고';
	html += '<br/>';
	html += '&nbsp;&nbsp;&nbsp;&nbsp;1) <span style="letter-spacing:0.1px;">공 고 명</span> : ' + $('#bidding_rebidding_title').val();
	html += '<br/>';
	html += '&nbsp;&nbsp;&nbsp;&nbsp;2) 재공고방법 : ' + $('#bidding_rebidding_way').val();
	html += '<br/>';
	html += '&nbsp;&nbsp;&nbsp;&nbsp;3) 재공고기간 : ' + $('#bidding_period').val();
	html += '<br/>';
	html += '<br/>';
	...
	html += '<br/>';
	html += '<span style="display:inline-block;width:40px;"></span>3. 입찰조서 1부.  끝.';
	html += '<br/>';
	html += '<br/>';
	html += '</div>';
	return html;
...
<div class="pop_sign_wrap" style="width:999px;">
    ...
    <div class="pop_sign_con scroll_on" style="padding:62px 16px 20px 16px;">  
        			...
        			<colgroup>
       				...
       				<tbody>
       					...
       					<tr>
       						...
       						<th>입찰결과</th>
       						<td style="text-align: left;padding-left: 5px;">
       							<select id="bidding_result" style="width: 85px;">
       								<option value="1">무응찰</option>
       								<option value="2">단독응찰</option>
       							</select>
       						</td>
       					</tr>
       					<tr>
       						...
       						<td colspan="3" style="text-align: left;padding-left: 5px;">
       							<input type="text" id="bidding_reason" style="width: 97%;" value="우리 원 구매계약규칙 제18조1항"/>
       						</td>
       					</tr>
       				</tbody>
        		</table>
        ...
        <div class="h2_btn3 mt10">
            <p class="tit_p mt10" id="">유찰에 따른 재공고</p>
        </div>
        <div id="biddingInfo">
        	<div class="com_ta2 hover_no mt10">
        		<table>
        			<colgroup>
       				...
       				<tbody>
       					<tr>
       						<th>공고명</th>
       						<td>
       							<input type="text" id="bidding_rebidding_title" style="width: 97%;"/>
       							<input type="hidden" id="bidding_type" value="130"/>
       						</td>
       						<th>재공고기간</th>
       						<td style="text-align: left;padding-left: 5px;">
       					...
       					<tr>
       						<th>재공고방법</th>
       						<td colspan="3" style="text-align: left;padding-left: 5px;">
       							<input type="text" id="bidding_rebidding_way" style="width: 97%;" value="나라장터 시스템, 공공기관 경영정보 공개시스템, 농정원 홈페이지"/>
       						</td>
       					</tr>
       					<tr>
