...
// 		          },
			dataBound: function(e){
                ...
                $(".progress").each(function(){
                    var row = $(this).closest("tr");
                    var model = grid.dataItem(row);
                    if(model.DIV_NM =="" && model.BOTTOM_NM != ""  &&  model.MGT_NM == ""){          //프로젝트 소계
                       $(this).kendoProgressBar({
                           change: function(e) {
                    ...
                    }else if(model.DIV_NM == "" && model.BOTTOM_NM == "" && model.MGT_NM != ""){           //하위사업
                        $(this).kendoProgressBar({
                            change: function(e) {
                                 this.progressWrapper.css({
                    ...
                    }else if(model.DIV_NM =="" &&  model.BOTTOM_NM == "" && model.MGT_NM == ""){            //최종
                       $(this).kendoProgressBar({
                            change: function(e) {
                                 this.progressWrapper.css({
...
// 			    },
		        ...
		        },
				{
		            template : function(dataItem) {
		            	if (dataItem.BOTTOM_CD == "" || dataItem.PJT_CD == "" || dataItem.DIV_CD == "") {
							return "<div style='text-align:right;'>" + numberFormatStr(dataItem.APPLY_AM2) + "&nbsp;</div>";
		            	}
		            	else {
		            		return "<div style='text-align:right;'>" + numberFormatStr(dataItem.APPLY_AM2) + "&nbsp;</div>";
		            	}
		        ...
		        },
		        {
		            template : function(dataItem) {
		            	if (dataItem.BOTTOM_CD == "" || dataItem.PJT_CD == "" || dataItem.DIV_CD == "") {
							return "<div style='text-align:right;'>" + numberFormatStr(dataItem.STATE_SUM) + "&nbsp;</div>";
		            	}
		            	else {
		            		return "<div style='text-align:right;'>" + numberFormatStr(dataItem.STATE_SUM) + "&nbsp;</div>";
		            	}
...
// 				            title: "집행률",
	...
	var mainDataSource = new kendo.data.DataSource({
		...
		transport: {
			read:  {
				url: "${pageContext.request.contextPath}/budget/execBudgetDataList",
				dataType: "json",
				type: 'post'
			},
...
// 				data.btmCd   = $('#btmCd').val();
	...
	function gridExcel() {
		...
		var ds = new kendo.data.DataSource({
			transport : {
				read : {
					url : _g_contextPath_+'/budget/execBudgetDataList?reqMonthFr='+reqMonthFr+'&reqMonthTo='+reqMonthTo+'&deptSeq='+deptSeq+'&divCd='+divCd+'&pjtCd='+pjtCd+'&btmCd='+btmCd,
					dataType : "json",
					type : 'GET'
				}
			},
...
// 		            	CHANGE_REASON: { type: "string" }
		...
		rows.push({
	      ...
	      ds.fetch(function(){
	        ...
	        for (var i = 0; i < data.length; i++){
	          ...
	          rows.push({
	            cells: [
	              ...
	              { value: data[i].MGT_NM, borderTop: { color: "#000000" },borderBottom: { color: "#000000" },borderLeft: { color: "#000000" },borderRight: { color: "#000000" } },
	              { value: data[i].BOTTOM_NM, borderTop: { color: "#000000" },borderBottom: { color: "#000000" },borderLeft: { color: "#000000" },borderRight: { color: "#000000" } },
	              { value: data[i].BGT_NM1, borderTop: { color: "#000000" },borderBottom: { color: "#000000" },borderLeft: { color: "#000000" },borderRight: { color: "#000000" } },
	              { value: data[i].BGT_NM, borderTop: { color: "#000000" },borderBottom: { color: "#000000" },borderLeft: { color: "#000000" },borderRight: { color: "#000000" } },
	              { value: data[i].deptNm, borderTop: { color: "#000000" },borderBottom: { color: "#000000" },borderLeft: { color: "#000000" },borderRight: { color: "#000000" } },
	              ...
	              { value: data[i].APPLY_AM2, borderTop: { color: "#000000" },borderBottom: { color: "#000000" },borderLeft: { color: "#000000" },borderRight: { color: "#000000" } },
	              { value: data[i].STATE_SUM, borderTop: { color: "#000000" },borderBottom: { color: "#000000" },borderLeft: { color: "#000000" },borderRight: { color: "#000000" } },
	        ...
	        kendo.saveAs({dataURI: workbook.toDataURL(), fileName: "사업별 예실대비 현황.xlsx"});
		...
		<div class="sub_contents_wrap">
			...
			<div class="top_box">
				<dl>				    
					...
					<dd>
						<input type="text" id="divNm" style="width: 300px" value="(재)전라북도생물산업진흥원" readonly />
				...
				<dl>
					...
					<dd>
						<input type="text" id="pjtNm" style="width: 500px" value="생물산업지원" readonly />
						<input type="button" id="pjtSearch" value="선택" />
						<input type="button" id="pjtSearchCncl" value="선택취소" />
						<input type="hidden" id="pjtCd" name="divCd" value="A1000|" />
					</dd>
