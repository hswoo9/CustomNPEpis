...
$(function(){
    ...
    fnTpfComboBoxInit('TR_PLACE', 'trPlace', $('#purcReqInfo'));
	...
	$("input[name=contType]").bind({
		click:function(){
			fnContTypeCheck(this);
		}
	});
	$('#payType').bind({
		change : function(){
	...
	$("input[name=contType]:checked").trigger("click");
	
	fnPayTypeChange();
	
	fnResizeForm();
	//setTimeout(function(){fnResizeForm2();}, 100);
...
function fnPurcReqTypeCheck(){
	var text1 = "기초견적서, 비교견적서, ";
	var text2 = "규격서 등";
	if($("input[name=contType]:checked").val() == "002"){
		text1 = "계약견적서, 사업자등록증, 기초견적서, 비교견적서, ";
	}
	if(purcReqType == "3"){
		text2 = "시방서 등";
	...
	$("#text_attach").html(text1 + text2);
}

function fnContTypeCheck(obj){
	if($(obj).val() == "001"){
		$(".contType2").hide();
		$("#payCon").data("kendoComboBox").value("001");
		$("#payType").data("kendoComboBox").value("001");
		$("#payCnt").val("1");
		$("#contDate").data("kendoDatePicker").value("");
		$("#basicAm").val("");
		$("#rate").val("");
		$('#attach2').show().removeClass('notUsed');
		$('#attach3').show().removeClass('notUsed');
		$('#attach4').hide().addClass('notUsed');
	}else if($(obj).val() == "002"){
		$(".contType2").show();
		$('#attach2').hide().addClass('notUsed');
		$('#attach3').hide().addClass('notUsed');
		$('#attach4').show().removeClass('notUsed');
	}
	fnPurcReqTypeCheck();
}
					...
					<tbody>
						<tr>
							...
							<td class="le" colspan="3">
								<input type="text" id="purcReqType" value="물품구매계획" readonly="readonly" disabled="disabled" code="1" style="width: 97%;"/>
								<input type="hidden" id="selectDocu" value="지출품의서" code="1"/>
								<input type="hidden" id="purcReqId"/>
							</td>
							<th>계약방법</th>
							<td colspan="3">
								<input type="radio" name="contType" id="contType1" value="001" text="입찰" checked="checked"><label for="contType1">입찰</label>
								&nbsp;
								<input type="radio" name="contType" id="contType2" value="002" text="수의"><label for="contType2">수의</label>
							</td>
<!-- 							<th>구매의뢰번호</th> -->
<!-- 							<td class="le" colspan="3"> -->
...
<!-- 							</td> -->
						...
						<tr>
							<th><img src="<c:url value='/Images/ico/ico_check01.png'/>" alt="" /> 제목</th>
							<td class="le" colspan="3">
						...
						<tr class="contType2" style="display:none;">
							...
							<th><span class="contType2">거래처</span></th>
							<td id="purc-trade-td" class="le" colspan="3">
								<span class="contType2">
									<input type="text" style="width:80%" id="txt_TR_NM"  class="txt_TR_NM non-requirement" tabindex="20001" readonly="readonly"/> 
							...
							<th style="display:none;">기초금액</th>
							<td style="display:none;">
								<input type="text" id="basicAm" class="ri"/>
							</td>
							<th style="display:none;">낙찰률</th>
							<td style="display:none;">
						...
						<tr>
							...
							<th style="display:none;"><span class="contType2">소재지</span></th>
							<td style="display:none;">
								<span class="contType2">
		...
		<div class="com_ta2 hover_no">
	    	<table style="border-top: 0px;">
				...
				<tr  id="attach1">
					<th><img src="<c:url value='/Images/ico/ico_check01.png'/>" alt="" /> 사업계획서</th>
					<td colspan="2" class="le" id="fileArea1"></td>
					<td>
						<input type="button" onclick="fnFileOpen(1)" 	value="업로드" class="file_input_button ml4 normal_btn2" />					
					</td>
				</tr>
				<tr  id="attach2">
					<th><img src="<c:url value='/Images/ico/ico_check01.png'/>" alt="" /> 제안요청서</th>
					<td colspan="2" class="le" id="fileArea2"></td>
					<td>
						<input type="button" onclick="fnFileOpen(2)" 	value="업로드" class="file_input_button ml4 normal_btn2" />					
					</td>
				</tr>
				<tr  id="attach3">
					<th><img src="<c:url value='/Images/ico/ico_check01.png'/>" alt="" /> 입찰공고문</th>
					<td colspan="2" class="le" id="fileArea3"></td>
					<td>
						<input type="button" onclick="fnFileOpen(3)" 	value="업로드" class="file_input_button ml4 normal_btn2" />					
					</td>
				</tr>
				<tr  id="attach4">
					<th><img src="<c:url value='/Images/ico/ico_check01.png'/>" alt="" /> 과업지시서</th>
					<td colspan="2" class="le" id="fileArea4"></td>
					<td>
						<input type="button" onclick="fnFileOpen(4)" 	value="업로드" class="file_input_button ml4 normal_btn2" />					
					</td>
				</tr>
			</table>
	    </div>
        ...
        <div class="com_ta2 ova_sc cursor_p" id="erpProjectInfo" style="overflow-y:scroll; height:75px;">
            ...
            <table>
				<tr id="erpProjectInfo-trsample" style="display: none;">            
	                ...
	                <td width="130">
	                	<span id="totalAM_h" class="totalAM_h" ></span>
	                </td>
	                <td width="100">
        ...
        <table id="erpBudgetInfo-tablesample" style="display:none">
		    <tr class="">         
		        <td width="270" id="budget-td">
		            ...
		            <input type="text" style="width:40%;display: none;" id="txt_BGTNM_REF" class="non-requirement" readonly="readonly" />
		            <input type="text" style="width:40%;display: none " id="txt_BUDGET_LIST"  class="requirement" tabindex="10001" readonly="readonly" />
