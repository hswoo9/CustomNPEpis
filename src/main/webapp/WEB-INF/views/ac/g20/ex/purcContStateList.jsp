			...
			columns : [
						{ 
							template: function(data){
								return "<input type='checkbox' id='check' class='checkbox' purcReqId='"+data.purcReqId+"' reqState='"+data.reqState+"' formId='"+data.cTikeycode+"' purcReqType='"+data.purcReqTypeCodeId+"' purcContId='"+data.purcContId+"' contStep='"+data.contStep+"'/>";
							}
							,width:50,
						},{
							field : "contStepNm",
						...
						{field : "purcReqType",
							  title : "구분",
// 							  template : function(data){
// 									var purcReqType = data.purcReqType;
// 									if(data.smallYn == "Y"){
// 										switch (data.purcReqTypeCodeId){
// 										case "1" :
// 											purcReqType = "소액물품구매";
// 											break;
// 										case "3" :
...
// 								}
							...
							title : "의뢰번호",
							template : function(data){
								return "<a href='javascript:fnDocPopOpen2("+data.cDikeycode+");' style='color: rgb(0, 51, 255);'>"+data.purcReqNo+"</a>";
							}
						},{
							...
							title : "제목",
							template : function(data){
								return "<a href='javascript:fnDocPopOpen2("+data.contDikey+");' style='color: rgb(0, 51, 255);'>"+data.purcReqTitle+"</a>";
							}
						},{
							field : "regDate",
							title : "의뢰일",
	...
	function fnPurcContVal(text){
		...
		for(var i = 0; i < checkboxArr.length; i++){
			if($(checkboxArr[i]).attr('contStep') != '003'){
				alert('상태가 \'계약보고\'인 건만 '+text+' 할 수 있습니다.');
				return false;
			}
		}
		return result;
	...
	function fnPurcContApproval(){
		if(!fnPurcContVal('계약체결'))return;
		if(confirm('계약을 체결합니다.')){
			...
			var purcContId = checkbox.attr('purcContId');
			purcContFormPop(purcReqId, purcReqType, formId, purcContId);
		}
	}
	
	function purcContFormPop(purcReqId, purcReqType, formId, purcContId){
		var url = _g_contextPath_ + '/Ac/G20/Ex/purcContConcForm.do?focus=txt_BUDGET_LIST&form_tp=tpfContConc&purcReqId=' + purcReqId + '&purcReqType=' + purcReqType + '&form_id=' + formId + '&purcContId=' + purcContId;
		var pop = "" ;
	...
	<div class="sub_contents_wrap">
		...
		<div class="top_box">
			...
			<dl>
				...
				<dt class="ar" style="width: 80px">계약금액</dt>
				<dd>
		...
		<div class="btn_div mt10 cl">
			...
			<div class="right_div">
				<div class="controll_btn p0">
					<button type="button" id="btnReturn" onclick="fnPurcContApproval();" style="margin-right: 20px;">계약체결</button>
