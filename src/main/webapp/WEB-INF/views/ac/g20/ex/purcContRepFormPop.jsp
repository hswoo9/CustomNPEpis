...
$(function(){
	...
	$('.pop_sign_wrap input').not($('#purcContInfo input, #erpTradeInfo-tablesample input')).prop('disabled', true);
    //품의/결의일자 컨트롤 초기화
...
// 		$('.purcReqType2hide').show();
	...
	$('.title_NM').html($('#purcReqType').val().replace('의뢰서','계약체결보고'));
	...
	fnTpfComboBoxInit('TR_PLACE', 'trPlace', $('#purcContInfo'));
	
	abdocu.TradeInfo.eventHandlerMapping($("#addTradeTd"), "_add")
	$("#addTradeBtn").on({
		click : function(){
			$("#txt_TR_NM_add").trigger("dblclick");
		}
	})
...
var evalDataSource = new kendo.data.DataSource({
    ...
    transport: { 
        read:  {
            url: _g_contextPath_ + '/Ac/G20/Ex/selectPurcReqBiddingEval.do',
            dataType: "json",
            type: 'post'
        },
      	parameterMap: function(data, operation) {
    ...
    schema: {
      data: function(response) {
	    return response.result.committeScoreList;
      },
      total: function(response) {
	    return response.totalCount;
	  }
...
function userPopInit(){
	...
	$(document).on('dblclick', '#userList .k-grid-content tr', function(){
		...
		$('#requester').val(gData.dept_name +' '+ gData.emp_name +' '+ gData.position_name);
		$("#requester_emp_seq").val(gData.emp_seq);
		$("#requester_dept_name").val(gData.dept_name);
		$("#requester_position").val(gData.position_name);
		$("#requester_duty").val(gData.duty_name);
...
function fnEvalInit(){
	$('#txt_TR_NM_cont').off('dblclick');
	$('#addTradeBtn').off('click').hide();
	$('#txt_TR_NM_cont').on({
		dblclick : function(){
			getEvalList();
		}
	...
	$('addTradeBtn').on({
		click : function(){
			$('#txt_TR_NM_cont').dblclick();
		}
	})
	
	$('#evalPopUp').kendoWindow({
		width: "600px",
	    title: '거래처 조회',
	    visible: false,
	    modal : true,
        ...
        columns: [
           {
               ...
               width: '100px',
               template: function(data){
            	   return '<input type="button" value="선택" onclick="fnEvalTrSelect(\'' + data.company_seq + '\')"/>';
               }
           }],
    }).data("kendoGrid");
}
...
function getEvalList(){
	$('#evalPopUp').data("kendoWindow").open().center();
	$("#evalList").data('kendoGrid').dataSource.read();
}

function fnEvalTrSelect(trCd){
	var data = {};
	...
	$.ajax({
		type: 'POST',
		async: false,
		url: _g_contextPath_  + '/Ac/G20/Ex/updateContTrEvalInfo.do',
		data: data,
		dataType: 'json',
		success: function(result) {
			var tradeInfo = result.result.tradeInfo;
			if(tradeInfo != 'Failed'){
				$('#txt_TR_NM_cont').val(tradeInfo.TR_NM);
				$('#txt_TR_NM_cont').attr('code', tradeInfo.TR_CD);
				$('#txt_ADDR_cont').val(tradeInfo.ADDR);
				$('#txt_ATTR_NM_cont').val(tradeInfo.ATTR_NM);
				$('#txt_TRCHARGE_EMP_cont').val(tradeInfo.TRCHARGE_EMP);
				$('#txt_JIRO_CD_cont').val(tradeInfo.JIRO_CD);
				$('#txt_REG_NB_cont').val(tradeInfo.REG_NB);
				$('#txt_BA_NB_H_cont').val(tradeInfo.BA_NB);
				$('#txt_CEO_NM_cont').val(tradeInfo.CEO_NM);
				$('#txt_DEPOSITOR_H_cont').val(tradeInfo.DEPOSITOR);
				$('#txt_TR_FG_cont').val(tradeInfo.TR_FG);
				$('#txt_TR_FG_NM_cont').val(tradeInfo.TR_FG_NM);
			}
			var biddingTSubList = result.result.biddingTSubList;
			$("#txt_TR_NM_add_span").html('');
			$.each(biddingTSubList, function(){
				var trCd = this.tr_cd;
				var trNm = this.tr_nm;
	...
	$('#evalPopUp').data("kendoWindow").close();
}
</script>
		...
		<div class="psh_btnbox">
			...
			<div class="psh_right">
				<div class="btn_cen mt8">
					<input type="button" class="psh_btn" id="btnApproval" value="계약체결보고" />
				</div>
			</div>
		</div>        
    </div>
...
<!-- 							</td> -->
							<th>의뢰자</th>
							<td class="le">
								<input type="text" id="requester" value="" readonly="readonly" style="width: 80%;"/>
								<input type="hidden" id="requester_emp_seq"/>
								<input type="hidden" id="requester_dept_name"/>
								<input type="hidden" id="requester_position"/>
								<input type="hidden" id="requester_duty"/>
							</td>
							<th>의뢰일자</th>
							<td class="le">
	    ...
	    <div class="h2_btn3 mt10">
            <p class="tit_p mt10" id="">계약체결보고</p>
        </div> 
	    <div id="purcContInfo">
			<div  class="com_ta2 hover_no mt10">
				<table>
					...
					<tbody>
						<tr>
							...
							<td class="le" colspan="3">
								<input type="text" id="contTitle" style="width: 97%;"/>
							</td>
							<th><img src="<c:url value='/Images/ico/ico_check01.png'/>" alt="" /> 거래처</th>
							<td colspan="3" id="cont-trade-td" class="le">
								<input type="text" style="width:70%" id="txt_TR_NM_cont"  class="txt_TR_NM non-requirement" tabindex="20001" readonly="readonly"/> 
								<input type="hidden" class="non-requirement txt_TR_FG" id="txt_TR_FG_cont" />                                                                  
								<input type="hidden" class="non-requirement txt_TR_FG_NM" id="txt_TR_FG_NM_cont"/>                                                                  
								<input type="hidden" class="non-requirement txt_ATTR_NM" id="txt_ATTR_NM_cont"/>                                                                  
								<input type="hidden" class="non-requirement txt_PPL_NB" id="txt_PPL_NB_cont"/>                                                                  
								<input type="hidden" class="non-requirement txt_ADDR" id="txt_ADDR_cont"/>                                                                  
								<input type="hidden" class="non-requirement txt_TRCHARGE_EMP" id="txt_TRCHARGE_EMP_cont" />                                                                  
								<input type="hidden" class="non-requirement txt_JIRO_CD" id="txt_JIRO_CD_cont"  />                                                                                        
								<input type="hidden" class="non-requirement txt_JIRO_NM" id="txt_JIRO_NM_cont"  />
								<input type="hidden" class="non-requirement txt_NDEP_AM" id="txt_NDEP_AM_cont"/>                                                                                               
								<input type="hidden" class="non-requirement txt_INAD_AM" id="txt_INAD_AM_cont"/>                                                                                               
								<input type="hidden" class="non-requirement txt_INTX_AM" id="txt_INTX_AM_cont"/>                                                                                               
								<input type="hidden" class="non-requirement txt_RSTX_AM" id="txt_RSTX_AM_cont"/>                                                                                               
								<input type="hidden" class="non-requirement txt_WD_AM" id="txt_WD_AM_cont" />
								<input type="hidden" class="non-requirement txt_ETCRVRS_YM" id="txt_ETCRVRS_YM_cont" />                                                                                               
								<input type="hidden" class="non-requirement txt_ETCDUMMY1" id="txt_ETCDUMMY1_cont" />                                                                                               
								<input type="hidden" class="non-requirement txt_DATA_CD" id="txt_DATA_CD_cont"  />                                                                                               
								<input type="hidden" class="non-requirement txt_ET_YN" id="txt_ET_YN_cont"/> 
								<input type="hidden" class="non-requirement txt_CTR_NM" id="txt_CTR_NM_cont" />
								<input type="hidden" class="non-requirement txt_CTR_CD" id="txt_CTR_CD_cont" />
								<input type="hidden" class="non-requirement txt_CTR_CARD_NUM" id="txt_CTR_CARD_NUM_cont" />
								<input type="hidden" class="non-requirement txt_BA_NB_H" id="txt_BA_NB_H_cont" />
								<input type="hidden" class="non-requirement txt_DEPOSITOR_H" id="txt_DEPOSITOR_H_cont" />
								<input type="hidden" class="non-requirement txt_BTR_NM_H" id="txt_BTR_NM_H_cont" />
								<input type="hidden" class="non-requirement txt_BTR_CD_H" id="txt_BTR_CD_H_cont" />
								<input type="hidden" class="non-requirement txt_REG_NB" id="txt_REG_NB_cont" />
						...
						<tr style="display: none;">
							<th class="">기초금액</th>
							<td class="le">
								<input type="text" id="basicAm" class="ri" disabled="disabled" style="width: 85%;" />
							</td>
							<th class="">예정가격</th>
							<td class="le">
								<input type="text" id="expectedAm" class="ri" style="width: 85%;" />
							</td>
							<td class="le purcReqType2hide" style="display: none;">
								<input type="text" id="payCon" class="payCon" style="width: 100px;"/>
							</td>
							<td class="le purcReqType2hide" style="display: none;">
								<input type="text" id="payType" class="payType" style="width: 85%;"/>
							...
							<td class="le" colspan="3" style="display: none;">
								<input type="text" id="contType" class="contType" style="width: 85%;"/>
							</td>
						</tr>
						<tr class="purcReqType2hide" style="display: none;">
							<th><span class="contType2">소재지</span></th>
							<td>
								<span class="contType2">
									<input id="trPlace" class="trPlace" style="width: 128px;"/>
								</span>
							</td>
							<th>낙찰률</th>
							<td class="le">
								<input type="text" id="rate" class="" disabled="disabled" style="width: 92%;" />
							</td>
						</tr>
						<tr>
							<th>부 거래처</th>
							<td colspan="6" id="addTradeTd" class="le">
						...
						<tr class="purcReqType2hide">
							...
							<td class="le" colspan="3">
								<span>(계약일로 부터</span>
								<input type="text" id="contEndDate" style="width: 120px;"/>
								<input type="text" id="contTerm" style="width: 50px;"/>
								<span>일 까지)</span>
							</td>
							<th>계약예정일</th>
							<td class="le">
								<input type="text" id="contDate" style="width: 120px;"/>
							</td>
							<th>계약금액</th>
							<td class="le">
								<input type="text" id="contAm" class="ri" disabled="disabled" style="width: 85%;" />
							</td>
						</tr>
						<tr>
							<th>첨부파일</th>
							<td colspan="6" id="fileArea_cont" class="le">
							</td>
							<td>
		...
		<div class="com_ta2 mt10">
			<table  id="erpBudgetInfo">
				<thead>
					<tr>
						<th width="100">예산회계단위</th>
						<th width="100">프로젝트</th>
			...
			<table style="width:966px;" id="erpTradeInfo">
				<thead>
					<tr>
			            ...
			            <th width="130"><img src="<c:url value='/Images/ico/ico_check01.png'/>" alt="" /> 조달청물품식별번호</th>      <!-- 1 -->                                                                                                     
			            <th width="120"><%-- <img src="<c:url value='/Images/ico/ico_check01.png'/>" alt="" /> --%>거래처</th>      <!-- 1 -->                                                                                                     
			            <th width="120">사업자번호</th>      <!-- 1 -->                                                                                                     
			            <th width="120">대표자명</th>      <!-- 1 -->         
			            <th width="100">품목구분</th>      <!-- 1 -->                                                                                                     
			            <th width="100"><img src="<c:url value='/Images/ico/ico_check01.png'/>" alt="" /> <span id="thItemNm"><%=BizboxAMessage.getMessage("TX000005413","품명")%></span></th>      <!-- 1 -->                                                                                                     
			            ...
			            <th width="80"><img src="<c:url value='/Images/ico/ico_check01.png'/>" alt="" /> <%=BizboxAMessage.getMessage("TX000000468","단가")%></th>      <!-- 1 -->                                                                                                          
		...
		<table id="erpTradeInfo-tablesample" style="display:none">
		    <tr class="">
		    	<td width="120"><input type="text" style="width:93%;" id=""  class="non-requirement" tabindex="20002" code="0"/></td>
				<td width="80"><input type="text" style="width:93%;" id=""  class="non-requirement" tabindex="20002" /></td>
				...
				<td width="130"><input type="text" style="width:93%;" id="ppsIdNo"  class="requirement" tabindex="20002" code="0"/></td>
...
<!-- 					<input type="text" style="width:70%" id="txt_TR_NM"  class="requirement" tabindex="20001"/>  -->
				...
				<td width="120"><input type="text" style="width:93%;" id="txt_REG_NB"  class="non-requirement" tabindex="20002" code="0"/></td>
				<!-- <td width="120"><input type="text" style="width:93%;" id="ceoNm"  class="non-requirement" tabindex="20002" code="0"/></td> -->
				<td width="120"><input type="text" style="width:93%;" id="txt_CEO_NM"  class="non-requirement" tabindex="20002" code="0"/></td>
...
<div class="pop_wrap_dir" id="evalPopUp" style="width: 600px; display: none;">
	<div class="pop_con">
		<div class="com_ta2">
			<div id="evalList"></div>
		</div>
