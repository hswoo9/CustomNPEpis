...
var selectedIdx = 1;
	...
	$('#loadingPop').parent().find('.k-window-action').css("visibility", "hidden");
		...
		//evidence.push({CMMN_DETAIL_CODE_NM : "보조금전용카드", CMMN_DETAIL_CODE : "004"});
...
//이체구분 선택
// TRANSFR_ACNUT_SE_CODE ( 이체구분 )
//SBSACNT_TRFRSN_CODE ( 사유 )
function fn_depositGb(e) {
	...
	if (CMMN_DETAIL_CODE == '001') { // 거래처 계좌로 이체 시
		SBSACNT_TRFRSN_CODE.wrapper.hide();
		$("#SBSACNT_TRFRSN_CN").hide();
	} else if (CMMN_DETAIL_CODE == '002') { // 보조금 계좌로 이체 시
...
function fn_initTextBox(dataItem){
	$("#SBSACNT_TRFRSN_CODE").data("kendoComboBox").select(0);
	var SBSACNT_TRFRSN_CODE = $("#SBSACNT_TRFRSN_CODE").data("kendoComboBox");
	var BCNC_SE_CODE= $('#BCNC_SE_CODE').val()
	SBSACNT_TRFRSN_CODE.wrapper.hide();
	$('#SBSACNT_TRFRSN_CN').hide();
	
	if (BCNC_SE_CODE == '001') { //법인사업자	
		fn_setCustomerGbCompany()
	} else if (BCNC_SE_CODE == '002') { //개인사업자
		fn_setCustomerGbCompany()
	} else if (BCNC_SE_CODE == '003') { //개인
		fn_setCustomerGbPerson();
	} else if (BCNC_SE_CODE == '004') { //해외	
		fn_setCustomerGbCompany()
	}
	
    var TRANSFR_ACNUT_SE_CODE = $("#TRANSFR_ACNUT_SE_CODE").data("kendoComboBox");
    TRANSFR_ACNUT_SE_CODE.value(TRANSFR_ACNUT_SE_CODE.value());
...
function setBankInfo() {
	...
	$.ajax({
			...
			success : function(result) {
				...
				$("#BCNC_BANK_CODE").val(result.BCNC_BANK_CODE);
...
function fn_setInfo(dataItem){
		...
		for(var i = 0; i<keys.length; i++){
			...
			if (key === "EXCUT_PRPOS_CN" || key === "PRDLST_NM") {
				$('#'+key).val("[" + $("#docNumber").val() + "] " + dataItem[key]);
			} else {
	...
	$.ajax({
		...
		success : function(result) {
 			if (result.OUT_YN == 'Y') {
				...
				selectedIdx = grid.select()[0].rowIndex + 1;
					
				dataItem.set('APPLY_DIV', '★');
				
				//원본 그리드 수정
				dataItem.set('BCNC_LSFT_NO', BCNC_LSFT_NO);
...
$(function() {
	...
	var accountGridDataSource = new kendo.data.DataSource({
		...
		transport : {
			read : {
				url : _g_contextPath_+"/kukgoh/accountGrid",
				dataType : "json",
				type : 'post'
			},
			parameterMap : function(data, operation) {
	...
	function fn_getAttaches() {
		var dt = $('#gisuDt').val().replace(/\-/g,'');
		
		var row = $("#accountGrid").data('kendoGrid').dataItem($("#accountGrid").data("kendoGrid").select());
		var targetId = dt+""+pad($('#gisuSeq').val(), 4) + ""+ $('#attachLnSeq').val();
		
		row.C_DIKEYCODE = "${data.C_DIKEYCODE}";
		row.targetId = targetId;	
		
		$.ajax({
			url : _g_contextPath_ + "/kukgoh/getEnaraAttaches",
			...
			success : function(result) {
				...
				if (result.flag === "SUCC") {
					...
					alert("이미 가져온 데이터 이거나 가져올 데이터가 없습니다.");
				}
			}
		})
								...
								<div class="top_box gray_box">
									<dl >
										...
										<dd>
											...
											<input type="hidden" id="PJT_CD" name="PJT_CD"  value=""/>
...
<!-- 										<dt style="width:100px; text-align:right;">작성일자</dt>
									...
									<dl class="next2">
										...
										<dd>
											<input style="width:625px" type="text" id="EXCUT_PRPOS_CN" name="EXCUT_PROPOS_CN" />
										</dd>
									</dl>
									<dl class="next2">
									<dt style="width:100px;"><img src="<c:url value='/Images/ico/ico_check01.png'/>" alt="" />품목</dt>
										<dd>
											<input style="width:625px" type="text" id="PRDLST_NM" name="PRDLST_NM" />
										</dd>
									</dl>
									<dl class="next2">
										<dt style="width:100px;"><img src="<c:url value='/Images/ico/ico_check01.png'/>" alt="" />증빙선택</dt>
										<dd>
											<input type="text" style="width:118px;" name="PRUF_SE_CODE" id="PRUF_SE_CODE" onchange="" placeholder="" />
										...
										<dd id="dd_invoice1" class = "" >
											<input type="text" style="width:290px;" id="PRUF_SE_NO" name="PRUF_SE_NO"  value='' readonly/>
									...
									<dl class="next2">
										...
										<dd style="width:200px;">
											...
											<!-- <input type="button"  class=""   style="width:102px; background: #ebebe4;color: #474242; border:;"name="getAttaches" id="getAttaches"  onclick="fn_getAttaches(this);" value="파일가져오기"> -->
										</dd>
										<dt style="width:100px;"><img src="<c:url value='/Images/ico/ico_check01.png'/>" alt="" />증빙일자</dt>
										<dd>
											<input type="text" id="EXCUT_REQUST_DE" name="EXCUT_REQUST_DE" />
										</dd>												
									</dl>
								</div>
							</div>	
							...
							<div class="com_ta" style="">
								...
								<div class="top_box gray_box">
									<dl class = "dt_etc">
										...
										<dd>
											<input style="width:128px" type="text" id="BCNC_CMPNY_NM" name="BCNC_CMPNY_NM" />
										</dd>
										<dt style="width:100px; text-align:right;" id="html_cmpny_no"><img src="<c:url value='/Images/ico/ico_check01.png'/>" alt="" />사업자등록번호</dt>
										<dd>
											<div id="html_div_lsft_no"><input style="width:128px" type="text" id="BCNC_LSFT_NO" name="BCNC_LSFT_NO" /></div>
											<div id="html_div_pin_no"><input style="width:75px" type="text" id="PIN_NO_1" name = "PIN_NO_1" maxlength="6"/> - <input style="width:18px" type="text" id="PIN_NO_2" name = "PIN_NO_2" maxlength="1"/>******</div>
										</dd>												
									</dl>
									<dl class="next2" id = "ownerInfo">
										<dt style="width:90px;">대표자명</dt>
										<dd>
											<input style="width:128px" type="text" id="BCNC_RPRSNTV_NM" name="BCNC_RPRSNTV_NM" />
										</dd>
										<dt style="width:90px; text-align:right;">전화번호</dt>
										<dd>
											<input style="width:128px" type="text" id="BCNC_TELNO" name="BCNC_TELNO" />
										</dd>										
									</dl>
									<dl class="next2 dt_etc" id="html_bcnc_cmpny_cnd_ind">
										<dt style="width:90px;">업태</dt>
										<dd>
											<input type="text" style="width:128px"  name="BCNC_BIZCND_NM" id="BCNC_BIZCND_NM" onchange="" placeholder="" />
										</dd>
										<dt style="width:90px;">업종</dt>
										<dd>
											<input type="text"  style="width:128px" name="BCNC_INDUTY_NM" id="BCNC_INDUTY_NM" onchange="" placeholder="" />
										</dd>
									</dl>
									<dl class="next2">
										...
										<dd>
											...
											<div  style="margin-top:5px;">
												<input type="text" style="width:645px;" id="BCNC_ADRES" name="BCNC_ADRES" />
									...
									<dl class="next2">
										...
										<dd>
											 <input type="text" id="BCNC_BANK_CODE_NM" name="BCNC_BANK_CODE_NM" /><img id="" style="margin-left:5px;" src="<c:url value='/Images/ico/ico_explain.png'/>" onclick="fn_backClick();"/>
										</dd>
										<dt style="width:78px;"><img src="<c:url value='/Images/ico/ico_check01.png'/>" alt="" />계좌번호</dt>
										<dd>
											<input type="text" id="BCNC_ACNUT_NO" name="BCNC_ACNUT_NO" />
										</dd>										
									</dl>		
									<dl class="next2">
										<dt style="width:90px;"><img src="<c:url value='/Images/ico/ico_check01.png'/>" alt="" />이체구분</dt>
										<dd>
											<input type="text" style="width:128px;" id="TRANSFR_ACNUT_SE_CODE" name="TRANSFR_ACNUT_SE_CODE" />
											<input type="text" style="width:180px;" id="SBSACNT_TRFRSN_CODE" name="SBSACNT_TRFRSN_CODE" />
											<input type="text" style="width:180px;" id="SBSACNT_TRFRSN_CN" name="SBSACNT_TRFRSN_CN" />
										</dd>										
									</dl>	
									<dl class="next2">
										<dt style="width:90px;">내통장표시</dt>
										<dd>
											<input type="text" id="SBSIDY_BNKB_INDICT_CN" name="SBSIDY_BNKB_INDICT_CN" />
										</dd>
										<dt style="width:100px;">받는통장표시</dt>
										<dd>
											<input type="text" id="BCNC_BNKB_INDICT_CN" name="BCNC_BNKB_INDICT_CN" />
										</dd>										
									</dl>																										
								</div>
							...
							<div class="pop_foot">
								<div class="btn_cen pt12">
									<input type="button" id="btnRequestResolution" class="" onclick="fn_validationCheck();" style="background:#1088e3; font-weight: bold; color:#fff;" value="집행정보 반영">
								</div>
							</div>												
						</div>
					</div>
