...
var $dataSource = new kendo.data.DataSource({		//그리드데이터소스
						...
						$.ajax({
							...
							success: function (rs) {
								...
								for(var j = 0; j < rs.list.length; j++){
									...
									if (docSeq === '17873' || docSeq === '44499' || docSeq === '43745' || docSeq === '53614' || docSeq === '65413') {
	...
	$(function(){
		...
		$("#from_period").kendoDatePicker({
		    ...
		    culture : "ko-KR",
			format : "yyyy-MM-dd",
			value : new Date('${year}','${mm-1}', (new Date().getDate()-7)),
		...
		var grid =  $("#grid").kendoGrid({
			...
			excelExport: function(e) {
				...
				$.each(e.workbook.sheets[0].rows, function(){
					if(this.type != 'header'){
						if(this.cells[4].value){
							this.cells[4].value = this.cells[4].value.toDate();
						}
						if(this.cells[5].value){
							this.cells[5].value = this.cells[5].value.toDate();
						}
						if(this.cells[11].value){
							this.cells[11].value = this.cells[11].value.toString().toMoney();
						}
					}
				});
			    e.workbook.fileName = "지출결의서 제출현황.xlsx";
			},
