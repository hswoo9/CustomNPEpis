...
<input type="hidden" id="mng" value="${param.mng}"/>
	...
	<p class="tit_p mt5 mt20">출장결과보고 반송</p>
...
<div class="pop_wrap_dir" id="returnReasonPopUp" style="width: 400px; display: none;">
	<div class="pop_con">
		<div class="com_ta2">
			<input type="text" id="returnReason" style="width: 100%;"/>
			<input type="hidden" id="bsSeq">
			<input type="hidden" id="abdocuNo">
		</div>
		<div class="controll_btn" style="text-align: center; padding-bottom: 0px;">
			<button type="button" onclick="bsrpReturn();">반송</button>
			<button type="button" onclick="bsrpPopClose();">닫기</button>
		</div>
	</div>
	<!-- //pop_con -->
</div>
        ...
        columns: [
            {
                ...
                width: 80,
                template : function(data){
                	var returnVal = '<div class="controll_btn p0 ac"><button type="button" id="" onclick="bsrpReturnPop(' + data.bs_seq + ', ' + data.abdocu_no + ');">반송</button></div>';
                	if(data.payment_dt){
                		returnVal = "";
                	}else if(data.return_yn == "Y" && data.report_st == 10){
                		returnVal = "임시저장";
                	}else if(data.return_yn == "Y" && data.report_st == 20){
                		returnVal = "결재중";
                	}else if(data.return_yn == "Y" && data.report_st != 90){
                		returnVal = '<div class="controll_btn p0 ac"><button type="button" id="" onclick="bsrpReturnCancel(' + data.bs_seq + ', ' + data.abdocu_no + ');">반송취소</button></div>';
                	}
                	return returnVal;
                },
            }
...
function returnReasonPopUpInit(){
	$('#returnReasonPopUp').kendoWindow({
		width: "400px",
	    title: '반송사유',
	    visible: false,
	    modal : true,
...
function bsrpReturnPop(bs_seq, abdocu_no){
	$('#returnReasonPopUp').data("kendoWindow").open().center();
	$("#bsSeq").val(bs_seq);
	$("#abdocuNo").val(abdocu_no);
}

function bsrpReturn(bs_seq, abdocu_no){
	if(confirm("출장결과보고를 반송합니다.")){
		var data = {};
		data.bs_seq = $("#bsSeq").val();
		data.abdocu_no = $("#abdocuNo").val();
		data.return_reason = $("#returnReason").val();
		$.ajax({
			url: "<c:url value='/bsrp/bsrpReturn.do' />",
			data : data,
			async : false,
			type : 'POST',
...
function bsrpPopClose(){
	$('#returnReasonPopUp').data("kendoWindow").close()
}

function bsrpReturnCancel(bs_seq, abdocu_no){
	if(confirm("출장결과보고를 반송취소합니다.")){
		var data = {};
		data.bs_seq = bs_seq;
		data.abdocu_no = abdocu_no;
		data.return_reason = "";
		$.ajax({
			url: "<c:url value='/bsrp/bsrpReturnCancel.do' />",
			data : data,
			async : false,
			type : 'POST',
			success: function(result){
