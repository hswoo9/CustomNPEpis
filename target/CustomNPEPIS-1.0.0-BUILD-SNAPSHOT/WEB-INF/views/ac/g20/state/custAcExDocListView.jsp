...
<script type="text/javascript" src='<c:url value="/js/kendoui/jszip.min.js"></c:url>'></script>
<script type="text/javascript" src='<c:url value="/js/neos/common.kendo.js"></c:url>'></script>
        ...
        pageSize: 10,
         transport: { 
             ...
             parameterMap: function(options, operation) {
            	 ...
            	 searchListCnt = Object.keys(formData).length;;
         ...
         schema:{
            data: function(response) {
         		for(var i=0;i<response.list.length;i++){
         			var date = response.list[i].ERP_GISU_DT;
         			var year = date.substr(0,4);
         			var month = date.substr(4,2);
         			var day = date.substr(6,2);
         			response.list[i].ERP_GISU_DT = year  + '-' + month + '-' + day;
         			response.list[i].APPLY_AM = response.list[i].APPLY_AM.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",");       			
         		}
...
//         dataBound: gridDataBound,
        columns: [
                ...
                ,{   field:"USERNAME",title:"<%=BizboxAMessage.getMessage("TX000000499","기안자")%>",width:100,headerAttributes : {style: "text-align: center;"},attributes: {style: "text-align: center;"} }
                ...
                ,{   field:"btr_nm",title:"입출금 계좌",width:100,headerAttributes: {style: "text-align: center;"},attributes: {style: "text-align: center;"}}
                ,{   field:"appr_end_date",width:100,title:"종결 일자",headerAttributes: {style: "text-align: center;"},attributes: {style: "text-align: center;"}}
            ],
...
// 	            			fnGetReferConferList();	
	            			...
	            			alert("<%=BizboxAMessage.getMessage("","품의서 조회권한이 없습니다.")%>");
	            		}
	            	}else{
...
docList.docView = function(dikeycode){
	...
	var uri = "http://gw.jif.re.kr/ea/edoc/eapproval/docCommonDraftView.do?multiViewYN=N&"+ param;
	openWindow3(uri,  "", '');
};
...
//     }
    ...
    if(confirm("<%=BizboxAMessage.getMessage("TX000013686", "삭제 시 사용자 데이터 복구는 불가능 합니다.　정말 삭제하시겠습니까?")%>".replace("　", "\n"))) {
    	$.ajax({
            type:"post",
//             url:'<c:url value="/Ac/G20/State/delAcExDoc.do" />',
            url:"http://gw.jif.re.kr/ea/Ac/G20/State/delAcExDoc.do",
            datatype:"json",
            data: tblParam,
            success:function(data){
...
<%--                            <c:if test="${delAuthor == 'Y'}" > --%>
                       		<!-- 엑셀 다운로드 버튼 -->
						   <button id="btnExcelDown" class="k-button">엑셀다운로드</button>
						    <!-- 문서 삭제 버튼 -->
