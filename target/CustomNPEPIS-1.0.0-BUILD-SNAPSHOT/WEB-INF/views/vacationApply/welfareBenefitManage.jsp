...
var $dataSource = new kendo.data.DataSource({		//그리드데이터소스
	...
	transport : {
		...
		parameterMap: function(data, operation) {
			...
			var sql_part3 = "FROM (SELECT * FROM dj_benefit_manage_batch where benefit_type = '2' AND payday BETWEEN '"+from+"' AND '"+to+"' GROUP BY emp_seq, family_name) AA LEFT JOIN ";
	...
	*/
	...
	function excel(){
	      ...
	      columns.push({width:100});
	        ...
	        kendo.saveAs({dataURI: workbook.toDataURL(), fileName: "복지포인트관리("+$("#apply_from_date").val().replace(/-/gi,'')+'-'+$("#apply_to_date").val().replace(/-/gi,'')+").xlsx"});
