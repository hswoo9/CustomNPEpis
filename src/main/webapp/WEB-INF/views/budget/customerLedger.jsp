...
var $dataSource = new kendo.data.DataSource({		//그리드데이터소스
	...
	transport : {
		read : {
			url : _g_contextPath_+ "/budget/ledgerList",
			dataType : "json",
			type : "post"
		},
...
$popDataSource = new kendo.data.DataSource({
	...
	transport : {
		read : {
			url : _g_contextPath_+ "/budget/getCustomerList",
			dataType : "json",
			type : "post"
		},
		parameterMap: function(data, operation) {
			
			data.NAME = firstFlag ? '데이터없음' : $("#pop_client_name").val();
	     	return data;
	     }
	},
	schema: {
	...
	function makeComboList(){
		$.ajax({
			url : _g_contextPath_+ "/budget/ledgerComboList",
			data : {
	...
	function dataBound(e){
		...
		if (grid.dataSource._data.length==0) {
			...
			var content = firstFlag ? '<tr class="kendo-data-row"><td colspan="' + colCount + '" class="no-data">거래처명을 검색해주세요.</td></tr>'
											 : '<tr class="kendo-data-row"><td colspan="' + colCount + '" class="no-data">데이터가 없습니다.</td></tr>'; 
			
			$(e.sender.wrapper)	.find('tbody').append(content);
		}
	};
		...
		<div class="com_ta">
			<div class="top_box gray_box" id = "dataBox">
				<dl>
					...
					<dd style="width:15%">
						<input type="text" style="width:80%" id ="accounting_title" disabled/>
					...
					<dt style="width:5%">거래처</dt>
					<dd style="width:15%">
						<input type="text" style="width:80%" id ="customer_name" disabled/>
						<button type="button" id ="customerPopBtn" value="검색">
						<img style="position:inherit;" src="<c:url value='/Images/btn/search_icon2.png'/>"></button>
