...
function fnBiddingInit(){
	$('input').not('#btnApprovalOpen, #biddingBeforeInfo input, #biddingInfo input').prop('disabled', true);
	$('.title_NM').html('전자시담');
	$('.psh_right').show();
	...
	$.ajax({
		...
		data:{purc_req_id : purcReqId, bidding_type : '130'},
		dataType : 'json',
		success : function(data) {
			...
			PURCBID3 = 'PURCBID3_' + purcReqId + '_130_' + data.biddingReferInfo.purc_req_bidding_id;
			$('#bidding_cont_date').val($('#purcReqDate').val());
			$('#bidding_cont_term').val($('#term').val());
		}
	});
	$.ajax({
		type : 'POST',
	...
	$.ajax({
		...
		async: false,
		url : _g_contextPath_  + '/Ac/G20/Ex/selectPurcReqBiddingRefer.do',
		data:{purc_req_id : purcReqId, bidding_type : '120'},
		dataType : 'json',
		success : function(data) {
			PURCBID2 = 'PURCBID2_' + purcReqId + '_120_' + data.biddingReferInfo.purc_req_bidding_id;
		}
	});
	$.ajax({
		type : 'POST',
		...
		success : function(data) {
			console.log(data);
			var committeInfo = data.result.committeInfo;
			if(committeInfo){
				var score = data.result.committeScoreList.length > 0 ? data.result.committeScoreList[0] : '';
				$('#bidding_trade').val(score.company_name);
				$('#bidding_point').val(score.TOTAL_SUM);
				if(committeInfo.EVAL_S_DATE === committeInfo.EVAL_E_DATE){
					$('#bidding_date').val(moment(committeInfo.EVAL_S_DATE).format('YYYY. MM. DD')+ '(' + moment(committeInfo.EVAL_S_DATE).format('ddd') + '), ' + committeInfo.EVAL_S_TIME + '~' + committeInfo.EVAL_E_TIME);
				}else{
					$('#bidding_date').val(moment(committeInfo.EVAL_S_DATE).format('YYYY. MM. DD')+ '(' + moment(committeInfo.EVAL_S_DATE).format('ddd') +  + ', ' + committeInfo.EVAL_S_TIME + ' ~ ' + moment(committeInfo.EVAL_E_DATE).format('YYYY. MM. DD')+ '(' + moment(committeInfo.EVAL_E_DATE).format('ddd') +  + ', ' + committeInfo.EVAL_E_TIME);
				}
				$('#bidding_place').val(committeInfo.EVAL_PLACE);
			}
		}
	});
// 	if(approKey){
...
function fnApproval(){
	...
	$.ajax({
		...
		success : function(data) {
		    ...
		    params.approKey = 'PURCBID4_' + purcReqId + '_' + $('#bidding_type').val() + '_' +  + data.purc_req_bidding_id;
		    params.outProcessCode = 'PURCBID4';
		    params.empSeq = $('#empSeq').val();
		    params.mod = 'W';
		    params.fileKey = makeDjFileKey();
		    onnaraFileToTemp(params.fileKey);
		    params.contentsStr = makeContentsStr();
		    params.title = $('#bidding_title').val() + ' 용역 전자시담 계획';
...
function makeContentsStr(){
	...
	html += '.';
	html += '<br/>';
	html += '2. ' + $('#bidding_title').val() + ' 용역의 가격 결정을 위한 전자시담을 실시하고자 합니다.';
	html += '<br/>';
	html += '<br/>';
	html += '&nbsp;&nbsp;가. 사업개요';
	html += '<br/>';
	...
	html += '<br/>';
	if($('#bidding_cont_date').val()){
		html += '&nbsp;&nbsp;&nbsp;&nbsp;2) 사업기간 : 계약 체결일로부터 ' + moment($('#bidding_cont_date').val()).format('YYYY.MM.DD');
	}else{
		html += '&nbsp;&nbsp;&nbsp;&nbsp;2) 사업기간 : 계약 체결일로부터 ' + $('#bidding_cont_term').val() + '일 까지';
	}
	html += '<br/>';
	html += '&nbsp;&nbsp;&nbsp;&nbsp;3) 사업금액 : 금' + $('#bidding_budget_am').val() + '원(금 ' + numToKOR($('#bidding_budget_am').val().toMoney2()) + '원) 부가세 포함';
	html += '<br/>';
	html += '&nbsp;&nbsp;&nbsp;&nbsp;3) 수행부서 : ' + $('#bidding_dept').val();
	html += '<br/>';
	html += '&nbsp;&nbsp;나. 적격심사';
	html += '<br/>';
	html += '&nbsp;&nbsp;&nbsp;&nbsp;1) 대<span style="display:inline-block;width:27px;"></span>상 : ' + $('#bidding_trade').val();
	html += '<br/>';
	html += '&nbsp;&nbsp;&nbsp;&nbsp;2) 평가일시 : ' + $('#bidding_date').val() + ' / ' + $('#bidding_place').val();
	html += '<br/>';
	html += '&nbsp;&nbsp;&nbsp;&nbsp;3) 평가방법 : ' + $('#bidding_way').val();
	html += '<br/>';
	html += '&nbsp;&nbsp;다. 적격심사 결과 : 적격(100점 만점 중 \'' + $('#bidding_point').val() + '\'점, 85점 이상일 경우 적격) 판정';
	html += '<br/>';
	html += '&nbsp;&nbsp;라. 적격심사 실시 : 수의계약';
	html += '<br/>';
	html += '&nbsp;&nbsp;&nbsp;&nbsp;1) 적용조항 : 국계령제27조제1항제2호(입찰자 또는 낙찰자가 없는 경우)';
	html += '<br/>';
	html += '&nbsp;&nbsp;&nbsp;&nbsp;2) 체결방법 : 국계령제30조제1항제1호의 경우에 해당';
	html += '<br/>';
	html += '&nbsp;&nbsp;&nbsp;&nbsp;3) 계약근거 : 적격심사 결과 적격으로 집계되어 해당 업체대상 전자시담 실시';
	html += '<br/>';
	html += '&nbsp;&nbsp;&nbsp;&nbsp;4) 시담기간 : 시담시작일로부터 2일간';
	html += '<br/>';
	html += '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <span style="font-size:9pt;">※ 재공고 후 1개 업체 참여에 따라 적격심사 실시</span>';
	html += '<br/>';
	html += '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <span style="font-size:9pt;">※ 시담 일시·기간은 최종결재일 등에 따라 일부 변동될 수 있음</span>';
	html += '<br/>';
	html += '<br/>';
	html += '<span style="display:inline-block;width:40px;">붙 임 </span>1. 입찰공고서 1부.';
	html += '<br/>';
	html += '<span style="display:inline-block;width:40px;"></span>2. 제안요청서 1부.';
	html += '<br/>';
	html += '<span style="display:inline-block;width:40px;"></span>3. 기술협상서 1부.  끝.';
	html += '<br/>';
	html += '<br/>';
	html += '</div>';
	return html;
       				...
       				<tbody>
       					<tr>
       						...
       						<td>
       							...
       							<input type="hidden" id="bidding_type" value="170"/>
       						</td>
       						<th>사업기간</th>
       						<td style="text-align: left;padding-left: 5px;">
       							<span>(계약일로 부터</span>
								<input type="text" id="bidding_cont_date"/>
								<input type="text" id="bidding_cont_term" style="width: 50px;"/>
								<span>일 까지)</span>
       						</td>
       					</tr>
       					<tr>
        			...
        			<colgroup>
       				...
       				<tbody>
       					<tr>
       						<th>대상</th>
       						<td>
       							<input type="text" id="bidding_trade" style="width: 97%;"/>
       						</td>
       						<th>평가일시</th>
       						<td>
       							<input type="text" id="bidding_date" style="width: 97%;"/>
       						</td>
       					</tr>
       					<tr>
       						<th>평가방법</th>
       						<td style="text-align: left;padding-left: 5px;">
       							<input type="text" id="bidding_way" style="width: 97%;" value="총 5명 평가위원(외부 5인)에 의한 적격심사"/>
       						</td>
       						<th>평가장소</th>
       						<td>
       							<input type="text" id="bidding_place" style="width: 97%;"/>
       						</td>
       					</tr>
       					<tr>
       						<th>결과</th>
       						<td colspan="3" style="text-align: left;padding-left: 5px;">
       							적격(100점 만점 중 ‘<input type="text" id="bidding_point" style="width: 50px;" />’점, 85점 이상일 경우 적격) 판정
       						</td>
       					</tr>
       				</tbody>
        		</table>
