...
acUtil.util.dialog.setData = function(paramMap, searchObj){
	...
	var trDblClickHandler =  function(dblClickparamMap, index){
        ...
        for(var i = 0, max = dblClickparamMap.length;i<max;i++){
        	...
        	if(tmpObj.id == 'txtDEPT_NM'){
        		userFlag = true;
        	}
        	if(tmpObj.id == 'txtDIV_NM'){
        		erpInfo = true;
        	}
        	if(tmpObj.id == 'txt_ProjectName'){
        		project = true;
        	}
        	
        	if(tmpObj.id == 'txtDEPT_NM' || tmpObj.id == 'dty_seq'){
        		ele.val(text + ' / ' + data.KOR_NM || "").attr("CODE", code|| "").attr('cocd', data.CO_CD);
        	}else{
        ...
        if(project){
        	$('#txt_BUDGET_LIST').val('').attr('code', '');
        	$('#BGT03_Txt').val('').attr('code', '');
        	$('#budget-td .non-requirement').val('').attr('code', '');
