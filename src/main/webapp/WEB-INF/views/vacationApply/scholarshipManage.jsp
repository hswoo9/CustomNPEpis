...
// 			url : _g_contextPath_+ "/vacationApply/getBenefitTypeList",
			url : _g_contextPath_+ "/vacationApply/getScholarshipManageList",
			dataType : "json",
			type : "post"
		},
...
// 			data.request_dept_seq = $("#empDept").val();
			...
			var sql_part3 = "FROM (SELECT * FROM dj_education_expense where AA.ACTIVE = 'Y' AND AA.status = '2' AND date_format(a.create_date,'%Y%m') BETWEEN '"+from+"' AND '"+to+"' GROUP BY request_emp_seq, target_name) AA LEFT JOIN ";
			...
			for (var i = 0; i <= yearTime*12+monthTime; i++) {
				...
				sql_part1 += ", IFNULL("+now_al+".scholarship_account,0) as 'scholarship_account"+now+"', IFNULL("+now_al+".date_format(a.create_date,'%Y%m'),'"+now+"') as 'date"+now+"'";
				sql_part2 += "IFNULL("+now_al+".scholarship_account,0)+";
				sql_part3 += "(SELECT * FROM dj_education_expense WHERE date_format(a.create_date,'%Y%m') = '"+now+"') "+now_al+" ON "+now_al+".target_name = AA.target_name  AND "+now_al+".request_emp_seq = AA.request_emp_seq LEFT JOIN";
				from_month++;
	...
	group : [
// 		{field : 'date', aggregates : [{field:'scholarship_account', aggregate:'sum'}, {field:'scholarship_account', aggregate:'max'}]},
		{field : 'request_emp_name', aggregates : [{field:'scholarship_account', aggregate:'sum'},{field:'scholarship_account', aggregate:'count'}]}
	],
	sort : [{field : 'date'}, {field : 'request_emp_name'}, {field : 'accumulate', dir:'asc'}],
	schema : {
	    ...
	    model : {
	    	fields : {
	    		scholarship_account : {type : 'number'}
	    	}
	    },
	},
    aggregate : [
//     	{field : 'scholarship_account', aggregate : 'sum'}
...
// 							return '';
// 						}else{
// 							return '총가족수 : ' +kendo.toString(e.scholarship_account.count,'n0');
// 						}
...
// 					format : "{0:n0}",
// 					groupFooterTemplate : function(e){
// 						return kendo.toString(e.scholarship_account.sum,'n0');
// 					},
// 					footerTemplate : function(e){
// 						return kendo.toString(e.scholarship_account.sum,'n0');
// 					}
...
// 			},
		...
		for (var i = 0; i <= yearTime*12+monthTime; i++) {
			...
			var now = ''+ from_year + from_month;
			var aggreList = {field:'scholarship_account'+now, aggregate:'sum'}
			...
			list.width = '10%';
			list.title = now;
			list.field = 'scholarship_account'+now;
			list.format = '{0:n0}';
			list.groupFooterTemplate = function(e){
				var key = 'scholarship_account'+monthArray[index];
				var value = e[key];
			...
			list.footerTemplate = function(e){var key = 'scholarship_account'+monthArray[index];
			var value = e[key];
			if(index == monthArray.length-1){
				index = 0;
			}else{
			...
			dataSource: new kendo.data.DataSource({		//그리드데이터소스
				...
				transport : {
					...
					parameterMap: function(data, operation) {
						...
						var sql_part1 = "SELECT TT.*, (CASE @emp WHEN TT.request_emp_seq THEN @sum:=@sum+TT.sum ELSE @sum:=sum END) accumulate, (@emp:=TT.request_emp_seq) emp FROM (SELECT AA.*";
						var sql_part2 = ", IFNULL(";
						var sql_part3 = "FROM (SELECT * FROM dj_education_expense where active = 'Y' AND status = '2' AND date_format(create_date,'%Y%m') BETWEEN '"+from+"' AND '"+to+"' GROUP BY request_emp_seq, target_name) AA LEFT JOIN ";
						var alphabet = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ';
						...
						for (var i = 0; i <= yearTime*12+monthTime; i++) {
							...
							var now = ''+ from_year + from_month;
							sql_part1 += ", IFNULL("+now_al+".scholarship_account,0) as 'scholarship_account"+now+"', IFNULL(date_format("+now_al+".create_date,'%Y%m'), '"+now+"') as 'date"+now+"'";
							sql_part2 += "IFNULL("+now_al+".scholarship_account,0)+";
							sql_part3 += "(SELECT * FROM dj_education_expense WHERE date_format(create_date,'%Y%m') = '"+now+"') "+now_al+" ON "+now_al+".target_name = AA.target_name  AND "+now_al+".request_emp_seq = AA.request_emp_seq LEFT JOIN";
							from_month++;
...
//			 		{field : 'payday', aggregates : [{field:'account', aggregate:'sum'}, {field:'account', aggregate:'max'}]},
					{field : 'request_emp_name', aggregates : aggregate}
				],
				sort : [{field : 'pay_day'}, {field : 'request_emp_name'}, {field : 'accumulate', dir:'asc'}],
				schema : {
					data : function(response){
						return response.list;
					},
...
// 				total += view[i].aggregates.scholarship_account.sum;
// 				var items = view[i].items;
// 				for (var j = 0; j < items.length; j++) {	//month
// 					var emp_name = items[j].value;
// 					var empTotal = items[j].aggregates.scholarship_account.sum;
// 					var items_items = items[j].items;
// 					empLength.push(items_items.length);
// 					for (var k = 0; k < items_items.length; k++) {	//emp
// 						excelData.push(items_items[k]);
// 					}
// 				}
// 			}
...
// 	      		if(i == 0){empChk = v.request_emp_name;};
// 	      		if(i == 0){paydayChk = v.date;};
// 	      		if(paydayChk != v.date){empChk = '';}
	      		
// 	      		if(empChk != v.request_emp_name){
// 	      			empSumData.push(empSum);
// 	      			empLength.push(empCount);
// 	      			empCount = 0;
// 	      			empSum = 0;
// 	      			empChk = v.request_emp_name;
// 	      		}
// 	      		empSum += v.scholarship_account;
// 	      		empCount++;

// 	      		if(paydayChk != v.date){
// 	      			paydaySumData.push(paydaySum);
// 	      			paydayLength.push(paydayCount);
// 	      			paydayCount = 0;
// 	      			paydaySum = 0;
// 	      			paydayChk = v.date;
// 	      		}
// 	      		paydaySum += v.scholarship_account;
// 	      		paydayCount++;
...
// 		    	  var rowData = [
// 		    		  { value: v.target_name,vAlign: "center", hAlign: "center", borderTop: { color: "#000000" },borderBottom: { color: "#000000" },borderLeft: { color: "#000000" },borderRight: { color: "#000000" } },
// 		    		  { value: v.target_relationship,vAlign: "center", hAlign: "center", borderTop: { color: "#000000" },borderBottom: { color: "#000000" },borderLeft: { color: "#000000" },borderRight: { color: "#000000" } },
// 		    		  { value: v.target_name,vAlign: "center", hAlign: "center", borderTop: { color: "#000000" },borderBottom: { color: "#000000" },borderLeft: { color: "#000000" },borderRight: { color: "#000000" } },
// 		    		  { value: v.accumulate,vAlign: "center", hAlign: "center", borderTop: { color: "#000000" },borderBottom: { color: "#000000" },borderLeft: { color: "#000000" },borderRight: { color: "#000000" } },
// 		    	  ];
...
// 		    	  if(empChk != v.request_emp_name){
// 		    		  empChk = v.request_emp_name;
// 		    		  rowData.unshift({ rowSpan: empLength[empCount],value: v.request_emp_name,vAlign: "center", hAlign: "center", borderTop: { color: "#000000" },borderBottom: { color: "#000000" },borderLeft: { color: "#000000" },borderRight: { color: "#000000" } });
...
// 		    	  if(paydayChk != v.date){
// 		    		  paydayChk = v.date;
// 		    		  rowData.push({ rowSpan: paydayLength[paydayCount],value: paydaySumData[paydayCount],vAlign: "center", hAlign: "center", borderTop: { color: "#000000" },borderBottom: { color: "#000000" },borderLeft: { color: "#000000" },borderRight: { color: "#000000" } });
// 		    		  rowData.unshift({ rowSpan: paydayLength[paydayCount],value: paydayChk,vAlign: "center", hAlign: "center", borderTop: { color: "#000000" },borderBottom: { color: "#000000" },borderLeft: { color: "#000000" },borderRight: { color: "#000000" } });
// 		    		  paydayCount++;
// 		    	  }
...
// 	}
	...
	function excel(){
	      ...
	      columns.push({width:100});
		    ...
		    $("#grid").data("kendoGrid").dataSource.sort();
		      ...
		      $.each(excelData, function(i,v){
		    	  ...
		    	  $.each(monthArray, function(a,b){
		    		  rowData.push({ value: v['scholarship_account'+b],vAlign: "center", hAlign: "center", borderTop: { color: "#000000" },borderBottom: { color: "#000000" },borderLeft: { color: "#000000" },borderRight: { color: "#000000" } });
		    	  })
	        ...
	        //save the file as Excel file with extension xlsx
	        kendo.saveAs({dataURI: workbook.toDataURL(), fileName: "학자금관리("+$("#apply_from_date").val().replace(/-/gi,'')+'-'+$("#apply_to_date").val().replace(/-/gi,'')+").xlsx"});
...
<div class="iframe_wrap">
	...
	<div class="sub_contents_wrap">
		<div class="btn_div mt10 cl">
			<div class="left_div">
				<p class="tit_p mt5 mb0">학자금지급액관리</p>
			</div>
			<div class="right_div">
