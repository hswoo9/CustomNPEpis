		...
		jQuery.ajax({
			...
			success : function(msg) {
				...
				if(msg.retValue == 0) {
                    ...
                    fingerAlert =setInterval(function() {
            			var blink = document.getElementById("imgDiv");
            			blink.style.visibility = blink.style.visibility == "" ? "hidden" : ""}, 700); // 깜빡이는 속도 조절 (클수록 느리게)
            ...
            success : function(msg) {
                ...
                if(msg.retValue == 0) {
                    ...
                    if ( msg.retVerify == 'Success' ) {
                    	...
                    	if ( ($('#userId').val() == '정국현'  ||  $('#userId').val() == 'khjeong') && (userIp == '192.168.72.111')) {
							getSecretKey();
						}else{
	                    	UnInit();
	                    	alert('등록한 지문이 아닙니다.');
