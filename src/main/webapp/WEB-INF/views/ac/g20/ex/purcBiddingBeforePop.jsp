...
function fnBiddingInit(){
	...
	$('.title_NM').html('사전 규격공고');
	$('.psh_right').show();
	$('#bidding_budget_am').val($('#contAm').val().toMoney());
	$('#bidding_title').val($('#purcReqTitle').val());
	var data = {purcReqId: purcReqId};
	$.ajax({
		type : 'POST',
		async: false,
		url : _g_contextPath_  + '/Ac/G20/Ex/selectPurcReqDept.do',
		data:data,
		dataType : 'json',
		success : function(data) {
			$('#bidding_dept').val((data.deptInfo.PARENT_DEPT_NAME ? data.deptInfo.PARENT_DEPT_NAME + ' ' : '') + (data.deptInfo.PURC_REQ_DEPT_NAME || ''));
			$('#bidding_dept_nm').val(data.deptInfo.PURC_REQ_DEPT_NAME);
			$('#bidding_dept_seq').val(data.deptInfo.PURC_REQ_DEPT_SEQ);
			$('#bidding_parent_dept_nm').val(data.deptInfo.PARENT_DEPT_NAME);
			$('#bidding_parent_dept_seq').val(data.deptInfo.PARENT_DEPT_SEQ);
...
function fnApproval(){
	...
	$.ajax({
		...
		success : function(data) {
		    ...
		    params.approKey = 'PURCBID1_' + purcReqId + '_' + $('#bidding_type').val() + '_' +  + data.purc_req_bidding_id;
		    params.outProcessCode = 'PURCBID1';
		    params.empSeq = $('#empSeq').val();
		    params.mod = 'W';
		    params.fileKey = makeDjFileKey();
		    onnaraFileToTemp(params.fileKey);
		    params.contentsStr = makeContentsStr();
		    params.title = $('#bidding_title').val() + ' 용역 사전 규격공고';
...
function makeContentsStr(){
	...
	html += '.';
	html += '<br/>';
	html += '2. ' + $('#bidding_title').val() + ' 용역의 사전 규격공고를 진행하고자 합니다.';
	html += '<br/>';
	html += '<br/>';
	html += '&nbsp;&nbsp;가. 공고 내용';
	html += '<br/>';
	...
	html += '<br/>';
	html += '&nbsp;&nbsp;&nbsp;&nbsp;2) 사업예산 : 금' + $('#bidding_budget_am').val() + '원(금 ' + numToKOR($('#bidding_budget_am').val().toMoney2()) + '원, VAT포함)';
	html += '<br/>';
	html += '&nbsp;&nbsp;&nbsp;&nbsp;3) 수행부서 : ' + $('#bidding_dept').val();
	html += '<br/>';
	html += '&nbsp;&nbsp;&nbsp;&nbsp;4) 공고방법 : ' + $('#bidding_way').val();
	html += '<br/>';
	html += '&nbsp;&nbsp;&nbsp;&nbsp;5) 규격기간 : ' + $('#bidding_period').val();
	html += '<br/>';
	html += '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span style="font-size:9pt;">' + $('#bidding_period2').val() + '</span>';
	html += '<br/>';
	html += '&nbsp;&nbsp;나. 향후계획 : ' + $('#bidding_plan').val();
	html += '<br/>';
	html += '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span style="font-size:9pt;">' + $('#bidding_plan2').val() + '</span>';
	html += '<br/>';
	html += '<br/>';
	html += '붙 임 제안요청서 1부.  끝.';
	html += '<br/>';
       				...
       				<tbody>
       					<tr>
       						...
       						<td>
       							...
       							<input type="hidden" id="bidding_type" value="110"/>
       						</td>
       						<th>사업예산</th>
       						<td>
       					...
       					<tr>
       						...
       						<td colspan="3"  style="text-align: left;padding-left: 5px;">
       							<input type="text" id="bidding_period" style="width: 97%;" value="공고시작일(결재완료일)로부터 5일간"/>
       							<br/>
       							<input type="text" id="bidding_period2" style="width: 97%; border-top: none; font-size: 7pt;height: 18px;" value="※ 기재부 계약예규(제383호, 2018.6,4) 제77조(구매규격 사전공개) 2항에 의거"/>
       						</td>
       					</tr>
       					<tr>
       						<th>향후계획</th>
       						<td colspan="3"  style="text-align: left;padding-left: 5px;">
       							<input type="text" id="bidding_plan" style="width: 97%;" value="사전규격에 따른 의견 반영 후 본 공고 실시"/>
       							<br/>
       							<input type="text" id="bidding_plan2" style="width: 97%; border-top: none; font-size: 7pt;height: 18px;" value="※ 공고 일시·기간은 최종결재일 등에 따라 일부 변동될 수 있음"/>
       						</td>
       					</tr>
       				</tbody>
        		</table>
