...
<input type="hidden" id="mng" value="${param.mng}"/>
	...
	<p class="tit_p mt5 mt20">출장신청서 환원</p>
				...
				<div class="btn_div">	
					<div class="right_div">
						<div class="controll_btn p0">
							<button type="button" onclick="paymentDtSave();" id="paymentDtBtn" >지급일 저장</button>										
...
//                		return data.payment_dt || "";
                ...
                width: 80,
                template : function(data){
                	var returnVal = '<div class="controll_btn p0 ac"><button type="button" id="" onclick="bsrpReturn(' + data.abdocu_b_no + ');">환원</button></div>';
                	if(data.confer_return > 0){
                		returnVal = '<div class="controll_btn p0 ac"><button type="button" id="" onclick="bsrpReturnCancel(' + data.abdocu_b_no + ');">환원취소</button></div>';
                	}else if(data.report_st == 10){
                		returnVal ="임시저장";
                	}else if(data.report_st == 20){
                		returnVal ="결재중";
                	}
                	return returnVal;
                },
...
function bsrpReturn(abdocu_b_no){
	if(confirm("예산환원 하시겠습니까?")){
		var data = {};
		data.isAllDocView = "0";
		data.ABDOCU_B_NO  = abdocu_b_no;
		$.ajax({
			url: "<c:url value='/Ac/G20/Ex/returnConferBudget.do' />",
			data : data,
			async : false,
			type : 'POST',
...
function bsrpReturnCancel(abdocu_b_no){
	if(confirm("예산환원취소 하시겠습니까?")){
		var data = {};
		data.ABDOCU_B_NO  = abdocu_b_no;
		$.ajax({
			url: "<c:url value='/Ac/G20/Ex/returnConferBudgetRollBack.do' />",
			data : data,
			async : false,
			type : 'POST',
			success: function(result){
