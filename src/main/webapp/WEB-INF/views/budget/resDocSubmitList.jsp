...
var authCode = '${userInfo.authorCode}';
...
var $dataSource = new kendo.data.DataSource({		//그리드데이터소스
	...
	$(function(){
		...
		var allDept = fnTpfGetDeptList();
		allDept.unshift({dept_name : "전체", dept_seq : ""});
		
		var deptCombo = $("#deptCombo").kendoComboBox({
		      dataSource: allDept,
		...
		$("#deptCombo").data('kendoComboBox').value($('#deptSeq').val());
		
		if(authCode.indexOf('RESDOCSUBMITALL') < 0){
			$('#deptComboArea').hide();
			$('#txtDept').show();
		}else{
			$('#deptComboArea').show();
			...
			columns : [
				...
				},
				{
					...
					template : function(dataItem) {
						...
						return dataItem.returnYn == "Y" ? "반송" : "미반송";
					},
					width : 10,
					title : "반송여부"
				},
			...
			<div class="top_box gray_box" id = "dataBox">
				<dl>
					...
					<dd style="width:150px;">
						<span id="deptComboArea"><input type="text" style="width:100%" id ="deptCombo" value="" /></span>
