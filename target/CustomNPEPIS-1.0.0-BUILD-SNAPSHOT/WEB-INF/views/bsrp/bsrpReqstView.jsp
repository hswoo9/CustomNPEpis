			...
			<div class="top_box gray_box">
				...
				<dl>
					...
					<dd style="line-height: 25px;width:10%;">
						...
						<input type="text" id="bs_des_txt" readonly="readonly" style="width: 60px;"/>
		...
		<div class="com_ta">
			<div class="top_box gray_box">
				<dl>
					...
					<dd style="line-height: 25px; width:30%;">
						<input type="text" id="fare_txt" style="width: 200px;" class="requirement userInput">
						<input type="text" id="fare" style="width: 80px;" class="requirement sumData" readonly="readonly">원
					</dd>
					<dt style="width: 8%;">
						일비
					</dt>
					<dd style="line-height: 25px;width:25%;">
						<input type="text" id="daily_txt" style="width: 150px;" class="requirement userInput">
						<input type="text" id="daily" style="width: 80px;" class="requirement sumData" readonly="readonly">원
					</dd>
					<dt style="width: 8%;">
						통행료
					</dt>
				...
				<dl>
					...
					<dd style="line-height: 25px;width:30%;">
						<input type="text" id="room_txt" style="width: 200px;" class="requirement userInput">
						<input type="text" id="room" style="width: 80px;" class="requirement sumData" readonly="readonly">원
					</dd>
					<dt style="width: 8%;">
						식비
					</dt>
					<dd style="line-height: 25px;width:25%;">
						<input type="text" id="food_txt" style="width: 150px;" class="requirement userInput">
						<input type="text" id="food" style="width: 80px;" class="requirement sumData" readonly="readonly">원
						<div>
...
// 	Budget_Search();
	...
	$('.userInput').on('change', function(){
		...
		$(this).next().val(numberWithCommas($(this).val().replace(/[^0-9]/g,"")));
		totalSum();
	});
...
function autoCalcBtn(){
	...
	if(erpToEmpInfo == null || erpToEmpInfo == ''){
		alert('출장비가 없습니다. 관리자에게 문의하세요.');
		return
	}

	var i = erpToEmpInfo.travel_pd / 2;
	
	var travelPd = [i, erpToEmpInfo.travel_pd]; //일비
	var roomPd = erpToEmpInfo.room_pd; //숙박비
	var foodPd = erpToEmpInfo.food_pd; //식비
	var car = $('input[name=car]:checked').val();
	
	var startDt = moment($('#startDt').val());
	var endDt = moment($('#endDt').val());
	var giDt = endDt.diff(startDt, 'days');
	
	if(!startDt._isValid || !endDt._isValid){
		alert('출장기간을 선택해주세요.');
		return
	}
	...
	//운임계산
	if(trafficArray.ba_seq != null){
		$('#fare_txt').val(trafficArray.strtpnt_kr + ' ~ ' + trafficArray.aloc_kr + ' ' + trafficArray.tfcmn_kr + ' ' + trafficArray.pymntamt + '*' + trafficType + ' ' + trafficTypeNm);
		$('#fare').val( numberWithCommas( trafficArray.pymntamt * trafficType) );
	}
	
	//일비  일수 * 업무차량 사용하면 10000, 미사용 20000
	$('#daily_txt').val('기간 ' + (giDt+1) + '일 * ' + numberWithCommas(travelPd[car]) + '원');
	$('#daily').val( numberWithCommas((giDt+1) * travelPd[car] ) );
	
	//숙박비 출장일수가 2일이상일 경우 숙박제공일 경우만 비용없음 나머지 일 * tnrqkr
	if(giDt > 0 && $('#topSearchRoom').val() != 2){
		$('#room').val( numberWithCommas(giDt*roomPd) );
		$('#room_txt').val('기간 ' + giDt + '일 * ' + numberWithCommas(roomPd) + '원');
	}else{
		$('#room').val('');
		$('#room_txt').val('');
	}

	//식비 출장일수가 2일이상일 경우 숙박제공일 경우만 비용없음 나머지 일
	if(giDt > 0 && $('#topSearchFood').val() != 2){
		$('#food').val( numberWithCommas((giDt+1)*foodPd) );
		$('#food_txt').val('기간 ' + (giDt+1) + '일 * ' + numberWithCommas(foodPd) + '원');
	}else{
		$('#food').val('');
		$('#food_txt').val('');
	}
